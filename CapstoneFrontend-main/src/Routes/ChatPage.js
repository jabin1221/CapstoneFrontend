...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8080/room/getmessage", userData)
      .then((response) => {
  ...
  const chatsavedb = () => {
    ...
    axios
      .post("http://localhost:8080/room/create", userData)
      .then((response) => {})
      .catch((error) => {});
  };
  ...
  const start = () => {
    ...
    client = over(sock);
    client.connect({}, () => {
      client.subscribe("/private/message/" + nickname, onReceived);
    });
  };
              ...
              [...message1].map((mes) =>
                mes.senduser == nickname ? (
                  <div class="chat_row right">
                    <div class="chat_right chat">{mes.message}</div>
                    <div className="empty"></div>
                  </div>
                ) : (
                  <div class="chat_row left">
                    <div class="chat_left chat">{mes.message}</div>
                    <div className="empty"></div>
                  </div>
                )
              )}
            {Chats.get(userData.receiveuser) &&
              [...Chats.get(userData.receiveuser)].map((chat, index) =>
                chat.senduser == nickname ? (
                  <div class="chat_row right">
                    <div class="chat_right chat">{chat.message}</div>
                    <div className="empty"></div>
                  </div>
                ) : (
                  <div class="chat_row left">
                    <div class="chat_left chat">{chat.message}</div>
                    <div className="empty"></div>
                  </div>
                )
              )}
