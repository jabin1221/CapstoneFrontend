...
function DetailPage(props) {
  ...
  return (
    <Grid className={styles.main} max_width="950px" margin="0 auto">
      ...
      >
        <Grid className={styles.main} container spacing={2}>
          ...
          <Grid item xs={6}>
            <Grid className={styles.main}>
              <h4 className="pt-5"><b>{itemDetail && itemDetail.title}</b></h4><br />
              <h2><b>{itemDetail && itemDetail.itemprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</b>원</h2>
              <hr />
              <h5 className={styles.gray}><FontAwesomeIcon icon={faHeart} className={styles.gray} /> {itemDetail && itemDetail.favor}</h5> <br />
              <Grid padding="10px 10px 10px 20px" container spacing={2}>
                <Grid xs={2} className={styles.gray}>
                  <CircleIcon style={{ fontSize: "small" }} /> 상품
                </Grid>
                <Grid xs={10}>
                  <p> {itemDetail && itemDetail.itemname}</p>
                </Grid>
                <br />
              </Grid>
              <Grid padding="10px 10px 10px 20px" container spacing={2}>
                <Grid xs={2} className={styles.gray}>
                  <CircleIcon style={{ fontSize: "small" }} /> 판매자
                </Grid>
                <Grid xs={10}>
                  <p> {itemDetail && itemDetail.memberid}</p>
                </Grid>
                <br />
              </Grid>
              <Grid padding="10px 10px 10px 20px" container spacing={2}>
                <Grid xs={2} className={styles.gray}>
                  <CircleIcon style={{ fontSize: "small" }} /> 배송비
                </Grid>
                <Grid xs={10}>
                  <p> 배송비 별도</p>
                </Grid>
              ...
              {itemDetail && (itemDetail.memberid !== nickname && itemDetail.status === "판매중" ? (
                <>
                  ...
                  <Grid padding="80px 10px 10px 20px" container spacing={2}>
                    <Grid xs={4}>
                      <button
                        onClick={() =>
                          navigate(
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          <h5>    <TagOutlinedIcon /> 상품정보</h5>
          <hr />
          <p>상품설명:<b>{itemDetail && itemDetail.maintext}</b></p>
        ...
        <Grid item xs={6}>
          ...
          <p><StoreMallDirectoryOutlinedIcon /> 상점 정보</p>
          <hr />
          <Button
            variant="outlined"
